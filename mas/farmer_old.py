...
class FarmerAgent(Agent):
    ...
    class UserHandler(CyclicBehaviour):
        """
        this class use to handle text message from user, tranfer it to rasa, get rasa response then set it back to user 
        and sent data to other other agent.
        ...
        responses = await rasa_agent.handle_text(message, sender_id=sender)
        if rasa_responses:
        ...
        async def run(self):
            ...
            if msg_recv.body:        
                ...
                msg_type, reply= await rasabot(message)


        async def userpropose(intent, data)
